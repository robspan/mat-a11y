 ...
 */
function verifyAllStructures(checksDir) {
  const results = {
    passed: [],
    failed: [],
    skipped: []
  ...
  const checkFolders = fs.readdirSync(checksDir, { withFileTypes: true })
    .filter(d => d.isDirectory())
    .map(d => d.name);

  for (const checkName of checkFolders) {
    const checkDir = path.join(checksDir, checkName);

    // Find verify file (html or scss)
    const verifyHtml = path.join(checkDir, 'verify.html');
    const verifyScss = path.join(checkDir, 'verify.scss');

    let verifyFile = null;
    if (fs.existsSync(verifyHtml)) {
      verifyFile = verifyHtml;
    } else if (fs.existsSync(verifyScss)) {
      verifyFile = verifyScss;
    }

    if (!verifyFile) {
      results.skipped.push({ check: checkName, reason: 'No verify file found' });
      continue;
    }
